...
from fatia_agent.eval.metrics import identificacao, operacional, porcao
...
class TestCarimboDeRascunho:
    ...
    def test_split_de_ajuste_nunca_publica(self):
        ...
        texto = relatorio([foto(**ACERTO)] * MINIMO_PUBLICAVEL, split="dev")
        assert "NÃO É RESULTADO PUBLICÁVEL" in texto
        assert "**Recomendação" not in texto

    def test_execucao_cortada_por_limite_nunca_publica(self):
        texto = relatorio([foto(**ACERTO)] * MINIMO_PUBLICAVEL, truncado=True)
        assert "NÃO É RESULTADO PUBLICÁVEL" in texto
    ...
    def test_uma_foto_a_menos_que_o_minimo_ja_e_rascunho(self):
        assert motivo_de_rascunho(cabecalho(n_fotos=MINIMO_PUBLICAVEL - 1)) is not None
        assert motivo_de_rascunho(cabecalho(n_fotos=MINIMO_PUBLICAVEL)) is None
...
class TestVeredito:
    ...
    def test_metrica_nao_medida_nao_passa_no_portao(self):
        """Sem `kcal_100g` no rótulo não há erro de kcal, e "não medido" não pode
        ser tratado como aprovado — é como um portão passa vazio."""
        sem_kcal = [
            foto(previstos=[("arroz branco", 100, None)], rotulados=[("arroz branco", 100, None)])
        ] * MINIMO_PUBLICAVEL
        texto = relatorio(sem_kcal)

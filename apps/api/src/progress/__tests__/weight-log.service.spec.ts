...
describe('WeightLogService', () => {
  ...
  describe('list', () => {
    ...
    it('filters by date range when from and to are provided', async () => {
      ...
      // Use explicit times: date-only strings parse to UTC midnight, which can
      // surprise readers. Being explicit also documents that the service does
      // no boundary adjustment — it forwards what it receives.

...
`pnpm setup` runs `scripts/setup.sh`, which:
...
4. starts the Postgres container (`infra/docker-compose.yml`);
...
Postgres (and optionally Logto) run in Docker; API and Web run on the host with Turbo + hot reload.
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `pnpm setup`         | Run the full bootstrap (idempotent — safe to re-run).                                                                 |
...
**Port collisions (3000 / 3001 / 3002 / 3003 / 5432)**

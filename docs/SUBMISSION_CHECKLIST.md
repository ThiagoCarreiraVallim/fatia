...
| Item                                        | Issue |
| ------------------------------------------- | ----- |
| A1 — anotações das 106 tools                | #169  |     | A2 — org Team/Enterprise | ✅ resolvido (Team ativo) |
...
submissão não avança, e são **106 tools** afetadas.
Trabalho:
...
- [ ] Preencher nas 106 tools. A separação é mecânica pelo prefixo do nome: `get_*` / `list_*` / `search_*` / `explain_*` / `export_*` → `readOnlyHint`;
...
### A7. Exercitar as 106 tools de ponta a ponta 🟠 (#171)
...
- [ ] Rodar as 106 tools no [MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector).
- [ ] Rodar as 106 tools como **custom connector** no Claude.
...
1. **#169** (anotações das 106 tools) — único bloqueador puramente de código, e o passo Tools do portal trava nele.
...
4. **#91 + #171** (validar o fluxo no Claude; exercitar as 106 tools no Inspector e no Claude; conta de teste populada).

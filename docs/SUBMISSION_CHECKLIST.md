...
| Item                                        | Issue |
| ------------------------------------------- | ----- |
| A1 — anotações das 103 tools                | #169  |     | A2 — org Team/Enterprise | ✅ resolvido (Team ativo) |
...
submissão não avança, e são **103 tools** afetadas.
Trabalho:
...
- [ ] Preencher nas 103 tools. A separação é mecânica pelo prefixo do nome: `get_*` / `list_*` / `search_*` / `explain_*` / `export_*` → `readOnlyHint`;
...
### A7. Exercitar as 103 tools de ponta a ponta 🟠 (#171)
...
- [ ] Rodar as 103 tools no [MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector).
- [ ] Rodar as 103 tools como **custom connector** no Claude.
...
1. **#169** (anotações das 103 tools) — único bloqueador puramente de código, e o passo Tools do portal trava nele.
...
4. **#91 + #171** (validar o fluxo no Claude; exercitar as 103 tools no Inspector e no Claude; conta de teste populada).
